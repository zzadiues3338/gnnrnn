...
def train_epoch(args, model, device, nodeloader, year_XY, county_avg, year_avg_Y, cur_step, optimizer, epoch, writer=None):
    ...
    for year in all_years:  # list(year_XY.keys()):
        ...
        for batch_idx, (in_nodes, out_nodes, blocks) in enumerate(nodeloader):
            #print(f"  Batch {batch_idx}: in_nodes={len(in_nodes)}, out_nodes={len(out_nodes)}")
    ...
    n_batch = batch_idx+1
    if n_batch == 0:
        print(f"No batches processed for year {year}!")
...
def train(args):
    ...
    # print(f"County set from year_XY: {county_set}")
    ...
    for year in range(min_year, max_year + 1):
        X_list, Y_list, county_list = [], [], []
    
        for county in county_set:
            if year not in X_dict[county] or year not in Y_dict[county]:
                continue  # Skip if this county is missing data for this year
    
            X = X_dict[county][year]
            Y = Y_dict[county][year]
        ...
        if len(X_list) == 0:
            print(f"Skipping year {year} because no data available for any county")
            continue
    
        X_tensor = torch.tensor(np.stack(X_list)).to(device)
        Y_tensor = torch.tensor(np.stack(Y_list)).to(device)
        counties_tensor = torch.tensor(county_list).to(device)
    
        # Store the data — note that X and Y have a "sequence length" of 1 to match expected input shapes
        year_XY[year] = (X_tensor.unsqueeze(1),  # [num_counties, 1, num_features]
                         Y_tensor.unsqueeze(1),  # [num_counties, 1, num_targets]
                         counties_tensor)        # [num_counties]
    
    print("Years available in year_XY after processing:", list(year_XY.keys()))
    ...
    # if 2000 not in year_XY:
    #     raise ValueError("Year 2000 not found in processed data. Available years: {}".format(list(year_XY.keys())))
