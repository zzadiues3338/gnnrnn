...
def get_X_Y(data, args, device):
    if args.data_dir == "soybean_data_full.npz":
        ...
        # CHANGED: Directly filter to 2017-2021 for training
        data = data[(years_all >= 2017) & (years_all <= 2022) & (counties_all != 25019)]  # CHANGED
        print("After filtering", data.shape)
    ...
    # Compute average yield of each year (for logging only, not features)
    avg_Y = {}
    avg_Y_lst = []
    for year in range(min_year, max_year+1):
        avg_Y[year] = np.nanmean(Y[years == year, :], axis=0)
    ...
    # REMOVED: avg_Y[min_year-1] setup (because we're not adding it to features)

    # REMOVED: This entire block adding "last year's average yield" to X
    # Ybar = []
    # for year in years:
    #     Ybar.append(avg_Y[year-1])
    # Ybar = np.array(Ybar)
    ...
    # Compute mean and std over training years (2017-2021)
    known_years = data[:, 1] < 2022  # CHANGED: directly fitting to your year range
    ...
    # Compute average of each output (for normalization later)
    Y_mean = np.nanmean(Y[known_years], axis=0, keepdims=True)
    Y_std = np.nanstd(Y[known_years], axis=0, keepdims=True)
    args.means = torch.tensor(Y_mean, device=device)
    args.stds = torch.tensor(Y_std, device=device)
    ...
    # Create (county, year) → features/labels dictionaries
    X_dict = {}
    Y_dict = {}
    year_dict = {}
    county_dict = {}
    ...
    # Compute average features for each year (use if county data missing)
    year_avg = {}
    for year in range(min_year, max_year+1):
        year_dict[year] = np.array(year_dict[year])
        year_avg[year] = np.mean(year_dict[year], axis=0)

    # Compute county average features (can be used if we need early prediction)
    county_avg = {}
    for county in county_set:
        county_dict[county] = np.array(county_dict[county])
        county_avg[county] = torch.tensor(np.nanmean(county_dict[county], axis=0)).to(device)

    # Track which counties have data for which years
    avail_dict = {}
    for year in range(min_year, max_year+1):
        avail_dict[year] = []
        for j, county in enumerate(county_set):
    ...
    # Fill missing data using neighbors if necessary
    for year in range(min_year, max_year+1):
        for i, county in enumerate(county_set):
            if year not in X_dict[county]:
